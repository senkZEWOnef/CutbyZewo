"""Builds branded PDFs: a plain estimate/invoice breakdown, and the fuller
printable Client Package (invoice/material breakdown, project photos, and a
contract page with a deposit/balance split and a signature line the client
signs by hand after printing)."""
import os
...
from pdf_common import (
    PAGE_W, PAGE_H, MARGIN, HEADER_H, CONTENT_W,
    BRAND_DARK, BRAND_ACCENT, GREEN, GREEN_DARK, TAN, ROW_ALT, BORDER, TEXT_DARK, MUTED,
    LABELS, ITEM_TYPE_KEYS,
    _styles, _make_canvas, _draw_header_band, _item_table,
    build_doc_type_badge, build_total_box, build_meta_block,
)
...
def _photo_cell(path, styles, cell_w, max_h):
    max_w = cell_w - 16
    try:
...
def _doc_meta(job, estimate, lang):
    """Shared meta derivation: doc type/badge color, doc number, date, contact lines."""
    L = LABELS[lang]
    doc_type = L["invoice"] if estimate.get("estimate_type") == "invoice" else L["estimate"]
    badge_color = GREEN if doc_type == L["invoice"] else BRAND_ACCENT
    doc_number = str(estimate.get("id") or "")[:8].upper()
    created_at = estimate.get("created_at")
    date_str = created_at.strftime("%B %d, %Y") if hasattr(created_at, "strftime") else (created_at or "")
    contact_lines = [v for v in (job.get("phone"), job.get("email"), job.get("address")) if v]
    return doc_type, badge_color, doc_number, date_str, contact_lines


def build_estimate_pdf(job, estimate, items, totals, language="en"):
    """Returns a BytesIO PDF: branded header, client/doc meta, item tables
    grouped by material/hardware/labor, and a total box. No photos or
    contract page — the plain "download estimate" document.
    """
    ...
    doc_type, badge_color, doc_number, date_str, contact_lines = _doc_meta(job, estimate, lang)
    client_name = job.get("client_name") or "Client"
    total_amount = float(estimate.get("amount") or 0)

    story = []

    badge = build_doc_type_badge(doc_type, badge_color)
    title_row = Table(
        [[Paragraph(estimate.get("name") or L["estimate"], styles["title"]), badge]],
    ...
    if estimate.get("description"):
        story.append(Paragraph(estimate["description"], styles["cell_desc"]))
        story.append(Spacer(1, 14))
    ...
    story.append(build_total_box(L["total"], total_amount, CONTENT_W))

    footnote_bits = []
    if estimate.get("labor_rate"):
        footnote_bits.append(f"Labor Rate: ${float(estimate['labor_rate']):,.2f}/hour")
    if estimate.get("markup_percentage"):
        footnote_bits.append(f"Material Markup: {float(estimate['markup_percentage']):,.2f}%")
    if footnote_bits:
        story.append(Spacer(1, 6))
        story.append(Paragraph(" &nbsp;|&nbsp; ".join(footnote_bits),
                                ParagraphStyle("fn", fontName="Helvetica-Oblique", fontSize=8.5,
...
def build_client_package_pdf(job, estimate, items, totals, images, contract_terms,
    ...
    if valid_images:
        ...
        cols = 1 if len(valid_images) == 1 else 2
        gap = 14
        max_h = 340 if cols == 1 else 260
        cell_w = (CONTENT_W - gap * (cols - 1)) / cols
        grid_rows = []
        row = []
        for i, path in enumerate(valid_images):
            row.append(_photo_cell(path, styles, cell_w, max_h))
            if len(row) == cols:
